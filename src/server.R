...
shinyServer(function(input,output){
  ...
  output$plot3d2 <- renderRglwidget({ 
    try(rgl.close())
    # topcluster = names(which.max(sorted[1,]))
    db_filtered = merged[merged$`Eps 1 MinPts 3`,1:3]
  ...
  output$tb <- renderUI({
    if(is.null(data()))
      ...
      tabsetPanel(tabPanel("About file", tableOutput("filedf")),tabPanel("Data", dataTableOutput("table")),tabPanel("Summary", dataTableOutput("sum")),tabPanel("Cluster",dataTableOutput("cluster")),tabPanel("Classification",dataTableOutput("class")),tabPanel("Top 10 Clusters",dataTableOutput("sort")),tabPanel("3D Plot",rglwidgetOutput('plot3d')),tabPanel("Wood Only 3D",rglwidgetOutput('plot3d2', width = 500, height = 400)))
  })
})
